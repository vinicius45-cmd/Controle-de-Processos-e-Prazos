import { cdpApi, isMockApi } from '../config/api';
...
export const cdpService = {
  ...
  async login(nickname: string, passwordPlana: string): Promise<LoginResponse> {
    try {
      const response = await cdpApi.post<LoginResponse>('/geproc/api/auth/login', {
        email: nickname.trim(),
  ...
  async getUsuarios(): Promise<CdpUsuario[]> {
    try {
      const response = await cdpApi.get<CdpUsuario[]>('/CDP/api/usuarios');
      return response.data;
    } catch (e) {
      if (!isMockApi) throw e;
  ...
  async getPermissoesTree(idUsuario: number): Promise<any> {
    try {
      const response = await cdpApi.get('/CDP/api/usuarios/me/permissoes', {
