...
export const TipoDocumentoService = {
  async listar(filtro = '', apenasAtivos = true): Promise<TipoDocumento[]> {
    try { const { data } = await api.get<TipoDocumento[]>('/dom-tipo-documento'); return data; } catch (error) { if (!isMockApi) throw error;
  ...
  async cadastrar(dados: DadosTipoDocumento): Promise<TipoDocumento> {
    try { const { data } = await api.post<TipoDocumento>('/dom-tipo-documento', { ...dados, blAtivo: 'S' }); return data; } catch (error) { if (!isMockApi) throw error;
  ...
  async atualizar(id: number, dados: DadosTipoDocumento): Promise<TipoDocumento> {
    try { const { data } = await api.put<TipoDocumento>(`/dom-tipo-documento/${id}`, dados); return data; } catch (error) { if (!isMockApi) throw error;
  ...
  async desativar(id: number): Promise<void> {
    try { await api.patch(`/dom-tipo-documento/${id}/desativar`); } catch (error) { if (!isMockApi) throw error; const item = localMockTiposDocumento.find((x) => x.idTipoDocumento === id); if (item) item.blAtivo = 'N'; }
  }
};
